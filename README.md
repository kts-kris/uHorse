...
<p align="center">
  <strong>uHorse v4.5.1 当前正式发布线</strong>
</p>

<p align="center">
  <em>Hub 负责调度与通道接入，Node 负责本地执行与结果回传；当前仓库 HEAD 已收口为正式发布的 `v4.5.1`，主交付物为 `uhorse-hub` 与 `uhorse-node-desktop`。</em>
</p>
...
uHorse 当前对外正式发布口径是 **v4.5.1 Hub-Node 主线**。
...
当前文档以 **仓库里已实现并验证的行为** 为准，不再把旧版 `/health/live`、`/health/ready`、`/api/v1/auth/*`、`/api/v1/messages` 当作当前主线，也不把 `v4.5.1` 写成旧单体 Agent 平台回归。
...
| 文档 | 说明 |
|------|------|
| [CHANGELOG.md](CHANGELOG.md) | `v4.5.1` 发布事实、文档同步记录与当前非目标说明 |
...
| [RELEASE_NOTES.md](RELEASE_NOTES.md) | `v4.5.1` 发布说明 |
