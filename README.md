...
<p align="center">
  <em>Hub 负责调度与通道接入，Node 负责本地执行与结果回传。</em>
</p>
...
当前文档以 **仓库里已实现并验证的行为** 为准，不再把旧版 `/health/live`、`/health/ready`、`/api/v1/auth/*`、`/api/v1/messages` 当作当前主线。
...
| [CHANNELS.md](CHANNELS.md) | 通道现状，重点是 DingTalk Stream |
