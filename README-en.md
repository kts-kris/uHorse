...
- The channel abstraction now has a minimal `ReplyContext` reply contract: Hub can use the generic reply path for channels that declare `REPLY_CONTEXT`, while DingTalk AI Card / reaction / transient-handle lifecycle behavior remains inside the DingTalk adapter.
- Feishu is now wired as the second minimal Hub sample: webhook challenge, message event preparation, prepared inbound submission, and source-message reply-context replies are covered.
...
| Generic reply-context path | ✅ | `ReplyContext` + `Channel::reply_via_context` are wired into the Hub generic dispatcher, while DingTalk advanced processing handles remain in the dedicated adapter |
| Minimal Feishu inbound sample | ✅ | `/api/v1/channels/feishu/webhook` supports challenge / message event preparation and can enter the prepared inbound mainline |
