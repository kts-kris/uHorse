...
当前实现会循环显示通道菜单，您可以连续配置多个通道，直到主动选择“完成通道配置，继续下一步”。
...
> 当前限制：这里仅收集 DingTalk App 凭据，不会直接代替桌面端发起 pairing。要打通“节点通知 -> 钉钉用户”链路，请在 Node Desktop 中发起绑定并在 DingTalk 中确认；`channels.dingtalk.notification_bindings` 仅用于兼容 seed/fallback。
...
当前向导会在每次完成单个通道配置后重新显示菜单。您可以连续配置多个通道，最后选择“完成通道配置，继续下一步”。
