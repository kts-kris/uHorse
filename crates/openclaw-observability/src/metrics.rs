...
impl MetricsCollector {
    ...
    pub fn inc_messages_received(&self, channel: &str) {
        counter!("openclaw_messages_received_total", "channel" => channel.to_string()).increment(1);
        self.messages_received.fetch_add(1, Ordering::Relaxed);
    }

    /// 记录发送消息
    pub fn inc_messages_sent(&self, channel: &str) {
        counter!("openclaw_messages_sent_total", "channel" => channel.to_string()).increment(1);
        self.messages_sent.fetch_add(1, Ordering::Relaxed);
    }

    /// 记录工具执行
    pub fn inc_tool_executions(&self, tool: &str) {
        counter!("openclaw_tool_executions_total", "tool" => tool.to_string()).increment(1);
        self.tool_executions.fetch_add(1, Ordering::Relaxed);
    }

    /// 记录工具错误
    pub fn inc_tool_errors(&self, tool: &str, error_type: &str) {
        counter!("openclaw_tool_errors_total", "tool" => tool.to_string(), "error_type" => error_type.to_string()).increment(1);
        self.tool_errors.fetch_add(1, Ordering::Relaxed);
    }

    /// 记录工具执行时间
    pub fn record_tool_execution(&self, tool: &str, duration_ms: u64) {
        histogram!("openclaw_tool_execution_duration_ms", "tool" => tool.to_string())
            .record(duration_ms as f64);
    }

    /// 设置活跃会话数
    pub async fn set_active_sessions(&self, count: u64) {
        gauge!("openclaw_active_sessions").set(count as f64);
    ...
    pub fn record_api_latency(&self, endpoint: &str, latency_ms: u64) {
        histogram!("openclaw_api_latency_ms", "endpoint" => endpoint.to_string())
            .record(latency_ms as f64);
    }

    /// WebSocket 连接数变化
    pub async fn set_websocket_connections(&self, count: u64) {
        gauge!("openclaw_websocket_connections").set(count as f64);
    ...
    pub fn inc_cache_hits(&self, cache_type: &str) {
        counter!("openclaw_cache_hits_total", "type" => cache_type.to_string()).increment(1);
    }

    /// 记录缓存未命中
    pub fn inc_cache_misses(&self, cache_type: &str) {
        counter!("openclaw_cache_misses_total", "type" => cache_type.to_string()).increment(1);
    }

    /// 记录数据库查询时间
    pub fn record_db_query_duration(&self, query_type: &str, duration_ms: u64) {
        histogram!("openclaw_db_query_duration_ms", "type" => query_type.to_string())
            .record(duration_ms as f64);
    }
}
