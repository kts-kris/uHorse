...
impl PermissionManager {
    ...
    pub async fn replace_rules(&self, rules: Vec<PermissionRule>) {
        ...
        *self.rules.write().await = sorted_rules;
    }
    ...
    pub async fn handle_approval_response(
        ...
        if let Some(mut request) = pending.remove(request_id) {
            ...
            if approved {
                ...
                Ok(ApprovalResolution::Approved { request, task_id })
            } else {
                ...
                Ok(ApprovalResolution::Rejected { request, task_id })
            }
        } else {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_handle_approval_response_returns_task_id_and_removes_pending_request() {
        let temp = TempDir::new().unwrap();
        let workspace = Arc::new(Workspace::new(temp.path()).unwrap());
        let manager = PermissionManager::new(workspace, true);
        ...
        let approval = manager
            .create_approval_request(
                ...
                Command::Shell(ShellCommand::new("sh")),
                create_test_context(),
        ...
        assert!(manager.get_pending_approvals().await.is_empty());
    }

    #[tokio::test]
    async fn test_replace_rules_sorts_by_priority_descending() {
        let temp = TempDir::new().unwrap();
        let workspace = Arc::new(Workspace::new(temp.path()).unwrap());
        let manager = PermissionManager::new(workspace, true);
        ...
        manager
            .replace_rules(vec![
                PermissionRule::new("low", "low").with_priority(1),
                PermissionRule::new("high", "high").with_priority(10),
                PermissionRule::new("mid", "mid").with_priority(5),
            ])
            .await;
