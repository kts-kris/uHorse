...
use uhorse_core::{SessionStore, Result};
...
impl SessionStore for SessionStoreAdapter {
    ...
    async fn get_session(&self, id: &uhorse_core::SessionId) -> Result<Option<uhorse_core::Session>> {
        self.store.get_session(id).await
    }

    async fn get_session_by_channel(&self, channel: uhorse_core::ChannelType, channel_user_id: &str) -> Result<Option<uhorse_core::Session>> {
    ...
    async fn list_sessions(&self, limit: usize, offset: usize) -> Result<Vec<uhorse_core::Session>> {
