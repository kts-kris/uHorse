...
impl TelegramChannel {
    ...
    pub async fn handle_update_raw(&self, update_json: &str) -> Result<Option<(Session, Message)>> {
        ...
        let message_obj = update.get("message")
            ...
            .ok_or_else(|| OpenClawError::InternalError("No message in update".to_string()))?;

        let chat = message_obj.get("chat")
            .ok_or_else(|| OpenClawError::InternalError("No chat in message".to_string()))?;
...
impl Channel for TelegramChannel {
    ...
    async fn start(&mut self) -> Result<()> {
        ...
        if response.status().is_success() {
            ...
            return Err(OpenClawError::ChannelError(ChannelError::ConfigError("Telegram API auth failed".to_string())));
        }
