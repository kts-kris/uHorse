...
impl ConfigLoader {
    ...
    /// 加载配置
    pub fn load(&self) -> AnyhowResult<UHorseConfig> {
    ...
    /// 合并配置
    fn merge(&self, base: UHorseConfig, overlay: ConfigValue) -> AnyhowResult<UHorseConfig> {
    ...
    /// 应用环境变量覆盖
    fn apply_env_overrides(&self, config: &mut UHorseConfig) -> AnyhowResult<()> {
...
pub struct ConfigWatch {
    config: Arc<RwLock<UHorseConfig>>,
    loader: ConfigLoader,
    config_path: PathBuf,
}
...
impl ConfigWatch {
    ...
    pub fn new(loader: ConfigLoader, config_path: PathBuf) -> Self {
        Self {
            config: Arc::new(RwLock::new(UHorseConfig::default())),
            loader,
            config_path,
        }
    }
    ...
    /// 获取配置快照
    pub async fn get(&self) -> UHorseConfig {
