...
use openclaw_core::{DeviceManager, Result, DeviceId, DeviceInfo, OpenClawError};
...
impl DevicePairingManager {
    ...
    pub async fn initiate_pairing(
        ...
        let device_info = DeviceInfo {
            ...
            capabilities: openclaw_core::DeviceCapabilities::default(),
        };
    ...
    pub async fn get_request_by_code(&self, code: &str) -> Result<PairingRequest> {
        ...
        let request_id = codes.get(code)
            .ok_or_else(|| OpenClawError::InternalError("Invalid pairing code".to_string()))?;

        let requests = self.pairing_requests.read().await;
        let request = requests.get(request_id)
            .ok_or_else(|| OpenClawError::InternalError("Pairing request not found".to_string()))?;
    ...
    pub async fn confirm_pairing(&self, code: &str, user_id: String) -> Result<DeviceInfo> {
        ...
        if request.is_expired() {
            ...
            return Err(OpenClawError::InternalError("Pairing request expired".to_string()));
        }
        ...
        if let Some(device) = devices.get_mut(&request.device_id) {
            ...
            Err(OpenClawError::DeviceNotPaired(request.device_id.clone()))
        }
    }
    ...
    pub async fn reject_pairing(&self, code: &str) -> Result<()> {
        ...
        if let Some(request) = requests.get_mut(&request_id) {
            ...
            Err(OpenClawError::InternalError("Pairing request not found".to_string()))
        }
    }
    ...
    pub async fn get_pairing_status(&self, device_id: &DeviceId) -> Result<PairingStatus> {
        ...
        if let Some(device) = devices.get(device_id) {
            ...
            Err(OpenClawError::DeviceNotPaired(device_id.clone()))
        }
    }
}
    ...
    async fn pair_device(&self, id: &DeviceId) -> Result<()> {
        ...
        if let Some(device) = devices.get_mut(id) {
            ...
            *device = updated;
            ...
            Err(OpenClawError::DeviceNotPaired(id.clone()))
        }
    }
