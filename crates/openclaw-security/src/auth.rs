...
use openclaw_core::{AuthService, Result, DeviceId, AccessToken};
...
impl JwtAuthService {
    ...
    async fn create_access_token(
        ...
        .map_err(|e| openclaw_core::OpenClawError::InternalError(e.to_string()))?;
...
impl AuthService for JwtAuthService {
    ...
    async fn revoke_token(&self, token: &str) -> Result<()> {
        ...
        .map_err(|_| openclaw_core::OpenClawError::InvalidToken)?;
...
impl JwtAuthService {
    ...
    pub async fn verify_with_auto_refresh(&self, token: &str) -> Result<AccessToken> {
        match self.verify_token(token).await {
            ...
            Err(openclaw_core::OpenClawError::TokenExpired) => {
                // 尝试使用刷新令牌
