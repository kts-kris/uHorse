...
impl CronParser {
    ...
    pub fn parse(&self, expression: &str) -> Result<CronSchedule> {
        ...
        if parts.len() != expected_len {
            return Err(openclaw_core::OpenClawError::ScheduleConflict(format!(
    ...
    fn parse_field(expr: &str, min: u32, max: u32) -> Result<CronField> {
        ...
        if let Some(pos) = expr.find('/') {
            ...
            step = Some(expr[pos + 1..].parse::<u32>()
                .map_err(|_| openclaw_core::OpenClawError::ScheduleConflict(
        ...
        for part in expr.split(',') {
            if let Some(range_pos) = part.find('-') {
                ...
                if val < min || val > max {
                    return Err(openclaw_core::OpenClawError::ScheduleConflict(
