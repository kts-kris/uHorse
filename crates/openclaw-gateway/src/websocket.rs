...
use openclaw_core::{
    ...
    SessionId, Result, OpenClawError,
};
use axum::{
    extract::{
...
impl WebSocketHandler {
    ...
    async fn handle_message(
        ...
        let msg: ProtocolMessage = serde_json::from_str(text)
            .map_err(|e| OpenClawError::InvalidMessage(format!("Invalid JSON: {}", e)))?;
    ...
    async fn handle_handshake(req: HandshakeRequest) -> Result<Option<String>> {
        ...
        let response = HandshakeResponse {
            ...
            capabilities: openclaw_core::ServerCapabilities::default(),
            pairing_required: false,
            auth_status: openclaw_core::AuthStatus::Authenticated,
        };
    ...
    /// 创建错误响应
    fn create_error_response(error: &OpenClawError) -> String {
        let error_detail = ErrorDetail::new(error.code(), error.to_string());
        let response = openclaw_core::Response::err("error", error_detail);
